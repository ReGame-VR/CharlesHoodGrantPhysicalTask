...
%% !! PARAMETERS TO SET !!
dim = 3; %The width of the green target square
halfdim = dim / 2;
...
fprintf(VRdata, '\t trialNum, \t time, \t targetlNum, \t targetTime, \t Weightshift success, \t button success, \t random or set sequence \t X coord of target \t Y coord of target\n\n');
fclose(VRdata);
...
%                         set(scoreBox,'String',score)
                        ...
                        formatSpec = '\t %d \t %3.2f \t %d \t %3.2f \t %d \t %d \t %d \t %3.2f \t %3.2f\n';
...
%                              targetTime = toc;
%                          end
                        fprintf(VRdata, formatSpec, j, cputime, lightNum, targetTime, wghtScs, btnScs, decide, cog(1), -cog(2));
                        fclose(VRdata);
