...
%     %%%add path for the excel file that will be generated after each
%     %%%session
%     %general algorithm for writing to the excel file once specified: 
%     tableHeaders = {  };
%     filename = ;
%     sheet = ;
%     xlRange = 'A1' ;
...
%     mainDataToWrite = [   ];
%
%     xlswrite(filename, mainDataToWrite, sheet, xlRange)    
%     filename = ;
%     sheet = ;
%     xlRange = 'A2' ; 
...
%         DateString = datestr(dt);
        ...
        %%%%%%%%--------------   
            ...
            %%COMMENTED OUT WHILE EDITING TARGET PLACEMENT
%             %setting the coordinates for the targets on the balance board
...
%%%NOTE: add an error check bit for when sending and receiving packets from
%%%the master arduino
%%SERIAL SETUP
comPort = 'COM3'; %Set the com port the arduino is connected to here. To find that type instrfind
...
%look for a way to check the serial ports first to make sure 'COM14' is
%correct
...
%                         end
                            ...
                            % Separate values
                             for idx=1:numel(parts)
                                part = parts{1, idx};
                                 ...
                                 tick_time = (t_el / (ticks_in_green + ticks_in_yellow + ticks_in_red));
                   ...
                   %%%code fore updating gui score display 3/14:
                        ...
                        %fprintf(VRdata, '\n\n\t trialNum \t date \t time \t targetlNum \t targetTime \t Weightshift success \t button success \t random or set sequence \t X coord of target \t Y coord of target \t COPtotalpath \t ScorePerTarget \t ScorePerTrial \t CumulativeScore \t SecondsInRed \t SecondsInYellow \t SecondsInGreen\n\n');
                        format = '\t %d \t %s \t %f \t %d \t %f \t %d \t %d \t %d \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f \t %f\n';
                        VRdata = fopen(VRString,'a');
                        fprintf(VRdata, format, trialNum, date, toc(tStart), target_num, targetTime, wghtScs, btnScs, decide, targets(lightNum, 1), targets(lightNum, 2), COP_distance, score(i), score(i), cum_score, time_in_red, time_in_yellow, time_in_green);
                        fclose(VRdata);
