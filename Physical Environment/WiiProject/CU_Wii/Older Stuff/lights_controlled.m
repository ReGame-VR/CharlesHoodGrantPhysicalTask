        ...
        while quitgui == 0
            ...
            while go == 1
                ...
                if (sequence < .5)
                    ...
                    lights = randi([1,6],1,6);
                    gen = 0;
                end
                
                set = 1;
                for x = 1:length(lights)
                    lights(x)
                    check = 0;
                    
                    fprintf(arduinoCom, '%u', lights(x));
                    ...
                    while (now < timeout || check == 1)
                        data.bb.time(iter) = toc(tStart); % find the elapsed time now
                        cog = bb.wm.GetBalanceBoardCoGState(); % find the center of pressure
                        sensors = (bb.wm.GetBalanceBoardSensorState()-sensor_avg_upright)./C; % find the force on each sensor, calibrate
                        loop = cog(1);
                    
                        if (lights(x) <= 3)
                            if (cog(1) <= left)
                                check = 1;
                                fprintf(arduinoCom, '%u', 7);
                                pause(.2)
                                break
                            end
                        else
                    ...
                    if (check == 0)
                        fprintf(arduinoCom, '%u', 8);
                    end
                    pause(2)
                end %End of for
                pause(3)
            end %End of constant while
                    
                    
                   % while 1
                   %     input_time = fread(arduinoCom);
                   %     if input_time ~= 0
                   %        break
                   %     
                   %     end
