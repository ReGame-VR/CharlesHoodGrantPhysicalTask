    ...
    if first == 0
        ...
        if cal == 1
          ...
          %  [zeroX, zeroY, XMaxWeight, XMinWeight, YMaxWeight, YMinWeight] = getMaxWeight(bb); %weight shift bounds calibration
            ...
            fprintf(VRdata, '\n\n\t trialNum \t time \t targetlNum \t targetTime \t Weightshift success \t button success \t random or set sequence \t X coord of target \t Y coord of target \t COPtotalpath \n\n');
            fclose(VRdata);
...
halfdim = dim / 2;
devdim = dim * 2; %The width of the yellow target square
...
%                         set(scoreBox,'String',score)
                        ...
                        formatSpec = '\t %d \t %3.2f \t %d \t %3.2f \t %d \t %d \t %d \t %3.2f \t %3.2f \t %3.2f \n';
...
%                              targetTime = toc;
%                          end
                        fprintf(VRdata, formatSpec, j, cputime, lightNum, targetTime, wghtScs, btnScs, decide, cog(1), -cog(2), COP_length);
                        fclose(VRdata);
